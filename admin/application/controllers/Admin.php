...
class Admin extends CI_Controller {
   ...
   public function dashboard(){
        ...
        $where_website = array('reg_through'=>'0','status!='=>'3');
        $data['website'] = count($this->General_Model->GetFullData1("tbl_registration",$where_website));
        
        
        // $data['admin'] = count($this->General_Model->GetFullData($table = "tbl_registration",'reg_through','1','status!=','3'));
        $where_admin = array('reg_through'=>'1','status!='=>'3');
        $data['admin'] = count($this->General_Model->GetFullData1("tbl_registration",$where_admin));
        
        
        // $data['mobile'] = count($this->General_Model->GetFullData($table = "tbl_registration",'reg_through','2','status!=','3'));
        $where_mobile = array('reg_through'=>'2','status!='=>'3');
        ...
        // $where5 = array('status'=>'3','delete_date'=>date('Y-m-d'));
        ...
        $where8 = array('reg_through'=>'0','reg_date'=>date('Y-m-d'),'status!='=>'3');
        $data['daily_website'] = count($this->General_Model->GetFullData1("tbl_registration",$where8));
        $where9 = array('reg_through'=>'1','reg_date'=>date('Y-m-d'),'status!='=>'3','status'=>'1');
        $data['daily_admin'] = count($this->General_Model->GetFullData1("tbl_registration",$where9));
        $where10 = array('reg_through'=>'2','reg_date'=>date('Y-m-d'),'status!='=>'3');
   ...
   {  
        ...
        { 
            ...
            $UserDetails['annual_income'] = !empty($this->input->post('annual_income'))?$this->input->post('annual_income'):'0';
            ...
            $UserDetails2['profile_for'] = !empty($this->input->post('created_by'))?$this->input->post('created_by'):'NULL';
            $UserDetails2['name'] = !empty($this->input->post('name'))?$this->input->post('name'):'NULL';
            $UserDetails2['gender'] = !empty($this->input->post('gender'))?$this->input->post('gender'):'NULL';
            $UserDetails2['countryCode'] = !empty($this->input->post('country_code'))?$this->input->post('country_code'):'NULL';
            $UserDetails2['phone'] = !empty($this->input->post('phone'))?$this->input->post('phone'):'NULL';
            $UserDetails2['mail'] = !empty($this->input->post('email'))?$this->input->post('email'):'NULL';
            $UserDetails2['age'] = !empty($this->input->post('age'))?$this->input->post('age'):'NULL';
            $UserDetails2['height'] = !empty($this->input->post('height'))?$this->input->post('height'):'NULL';
            $UserDetails2['weight'] = !empty($this->input->post('weight'))?$this->input->post('weight'):'NULL';
            $UserDetails2['color'] = !empty($this->input->post('color'))?$this->input->post('color'):'NULL';
            $UserDetails2['marital_status'] = !empty($this->input->post('marital_status'))?$this->input->post('marital_status'):'NULL';
            $UserDetails2['physical_status'] = !empty($this->input->post('physical_status'))?$this->input->post('physical_status'):'NULL';
            $UserDetails2['mother_tongue'] = !empty($this->input->post('mother_tongue'))?$this->input->post('mother_tongue'):'NULL';
            $UserDetails2['blood_group'] = !empty($this->input->post('blood_group'))?$this->input->post('blood_group'):'NULL';
            $UserDetails2['religion'] = 2;
            $UserDetails2['caste'] = !empty($this->input->post('belief'))?$this->input->post('belief'):'NULL';
            $UserDetails2['address'] = !empty($this->input->post('address'))?$this->input->post('address'):'NULL';
            $UserDetails2['secondary_code'] = !empty($this->input->post('secondary_code'))?$this->input->post('secondary_code'):'NULL';
            $UserDetails2['native_place'] = !empty($this->input->post('city'))?$this->input->post('city'):'NULL';
            $UserDetails2['pincode'] = $this->input->post('pincode');
            $UserDetails2['native_district'] = !empty($this->input->post('district'))?$this->input->post('district'):'NULL';
            $UserDetails2['other_contact_number'] = $this->input->post('othercontactno');
            $UserDetails2['education'] = !empty($this->input->post('qualification'))?$this->input->post('qualification'):'NULL';
            $UserDetails2['occupation'] = !empty($this->input->post('job'))?$this->input->post('job'):'NULL';
            $UserDetails2['job_category'] = !empty($this->input->post('job_status'))?$this->input->post('job_status'):'NULL';
            $UserDetails2['annual_income'] = !empty($this->input->post('annual_income'))?$this->input->post('annual_income'):'NULL';
            $UserDetails2['registration_hobbies'] = !empty($this->input->post('hobbies'))?$this->input->post('hobbies'):'NULL';
            $UserDetails2['photo_visibility'] = !empty($this->input->post('photo_visibility'))?$this->input->post('photo_visibility'):'NULL';
            $UserDetails2['photopassword'] = !empty($this->input->post('photopassword'))?$this->input->post('photopassword'):'0';

                
             $UserDetails2['staff_id'] = $this->session->userdata('user_id_admin');
            $UserDetails2['about'] = "I'm ".$UserDetails['name'].". I'm ".$UserDetails['age']." Years old";
            $UserDetails2['status'] = "1";
            $UserDetails2['reg_through'] = "1";     
                
                
                
                $this->General_Model->UpdateData($table = 'tbl_registration',$column_name = 'id',$UserID,$UserDetails2);
            ...
            $data['qualification'] = $this->General_Model->GetFullData($table = "tbl_education");
            $data['country'] = $this->General_Model->GetFullData($table = "tbl_country");
            $data['job'] = $this->General_Model->GetFullData($table = "tbl_profession");
   ...
   public function change_status()
    {
      ...
      if($status == '1')
      {
        ...
        // echo $checkexist;
        if($checkexist <= 1){
            $data=array('status'=>$status,'reg_date'=>date('Y-m-d'));
            // echo "sts:".$status;die;
      ...
      {
        //   $data=array('status'=>$status,'delete_date'=>date('Y-m-d'));
          $data=array('status'=>$status,'reg_date'=>date('Y-m-d'));
          $this->General_Model->UpdateData($table = 'tbl_registration',$column_name = 'id',$id,$data);
      }
