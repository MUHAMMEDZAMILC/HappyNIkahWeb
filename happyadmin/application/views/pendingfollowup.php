...
<div class="app-wrapper">
            ...
            <div class="col-auto">
            <h6 style="color:darkgrey;">Pending followup</h1>
            </div>
            ...
            <!-- /.card-header -->
            <div class="card-body">
            <h1 class="app-page-title mb-0">Pending Followup</h1>

            <?php $todaydate = date('Y-m-d');?>
        
            <?php $user_id=$this->session->userdata('user_id_admin');?>
               
            <?php $today= $this->db->select('tbl_registration.*,tbl_followup.id as fid,tbl_followup.status as fstatus,tbl_followup.delete_status,tbl_followup.message as fmessage,tbl_followup.fdate as followdate')
                                 ->from('tbl_registration')
                                 ->join('tbl_followup','tbl_followup.uid=tbl_registration.id')
                            
                                 ->where("STR_TO_DATE(tbl_followup.fdate, '%d-%m-%Y') >", $todaydate) 
                                 ->where('tbl_followup.login_id',$user_id)
                                 ->where('tbl_followup.delete_status','Active')->get()->result_array();?>
           ...
           <?php $todayfollow =  $this->db->select('*')->from('tbl_followup')
                                 ->join('tbl_registration','tbl_registration.id=tbl_followup.uid')
                                  ->where("STR_TO_DATE(tbl_followup.fdate, '%d-%m-%Y') >", $todaydate) 
                                 ->where('tbl_followup.delete_status','Active')->where('tbl_followup.login_id',$user_id)->get();?>
   ...
   <!--  <p>This will be fade out after some time... <a href="" onclick="remove_cart('123')" id="id">test</a></p> -->
          ...
          <!--   <li style="list-style-type:none;">
            ...
            <!-- Modal for enter message -->
                ...
                <select id="status" class="form-control custom-select" name="status" required  style="padding-bottom: 6px;">
       ...
       value="<?php echo $this->session->userdata('user_id_admin');?>" id="session">
         ...
         <!--    <script src="<?php echo base_url() ?>assets/plugins/jquery/jquery.min.js"></script> -->
           ...
           <script type="text/javascript">
            ...
            var maxDays = 1; // Default maximum days
            
            // Adjust maxDays based on the selected option
            if (selectedOption === "Interested")
            {
            maxDays = 2;
            }
            
            // Destroy the previous datepicker instance
            $("#noSunday").datepicker("destroy");
            
            // Reinitialize datepicker with the updated date range
            $("#noSunday").datepicker({
