      ...
      <div class="tab-content" id="orders-table-tab-content">
                    ...
                    <div class="tab-pane fade show active" id="orders-all" role="tabpanel" aria-labelledby="orders-all-tab">
                        <div class="app-card app-card-orders-table shadow-sm mb-5">
                             ...
                             <form action="<?php echo base_url();?>admin/assign_target_form_mangalyam" method="post">
                                 
                    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">

                                 
    <?php $targetdata2=$this->db->select('*')->from('tbl_users1')
           ->where('user_id !=', 9)->where('user_type', 3)->get()->result_array();?>
          ...
          <input type="submit"  id="save-project-btn3"  name="submit" class="btn btn-primary" style="margin-top: 20px;margin-left: 48px; margin-top: 35px;color: #fff;">
                    ...
                    $targetdata = $this->db->select('tbl_target.user_id,tbl_target.sale_amount,
                    tbl_target.target_amount,tbl_target.status,tbl_users1.user_id,tbl_users1.user_name')
                    ->from('tbl_target')->join('tbl_users1', 'tbl_users1.user_id=tbl_target.user_id')
                    ->where('tbl_target.status','2')
                    ->get()->result_array();
