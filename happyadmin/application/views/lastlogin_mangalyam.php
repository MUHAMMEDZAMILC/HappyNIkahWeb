...
<link rel="stylesheet" href="<?php echo base_url() ?>assets/plugins/datatables-responsive/css/responsive.bootstrap4.min.css">
        ...
        <?php $date=date('Y-m-d');?>  
        <?php $count_login=$this->db->select('tr.id as trid,tr.happynikah_id,tr.reg_through,tr.name,tr.age,tr.hmangalyam,tr.gender,tr.status,tu.*')
        ->from('tbl_userlogin as tu')->join('tbl_registration as tr','tr.id=tu.user_id')
        ->where('date',$date)
        ->where('tu.is_online','1')
        ->where('tr.hmangalyam','1')
        ...
        <?php $date=date('Y-m-d');?>         
        <?php $data_login=$this->db->select('tr.id as trid,tr.happynikah_id,tr.reg_through,tr.reg_date,tr.name,tr.age,tr.hmangalyam,tr.gender,tr.status,tu.*')
