  ...
  <div class="app-content pt-3 p-md-3 p-lg-4">
    <div class="container">
      ...
      <div class="tab-content" id="orders-table-tab-content">
              ...
              <button type="button" class="btn btn-success" id="exportto_femalegoto" style="display: none;">Export to Excel</button>
                     </div>   </div>
...
<script>
  ...
  $(document).ready(function() {
    $('#filterForm').submit(function(e) {
      ...
      $.ajax({
        url: '<?php echo base_url('admin/get_date_female_goto'); ?>',
        method: 'post',
        data: formData,
        dataType: 'html',
        success: function(data) {
          $('#customerData').html(data);
          $("#exportto_femalegoto").show();
        }
      });
    });

 $('#exportto_femalegoto').click(function() {
  from_date = $('#from_date').val();
  to_date = $('#to_date').val();
    $.ajax({
        type: 'POST',
        url: '<?= base_url() ?>admin/exportToExcel_dategoto', // Check this URL path
        data: {
           from_date:from_date,
           to_date:to_date
        },
        success: function(response) 
        {
            ...
            var downloadUrl = '<?= base_url() ?>upload/female_list_dategoto.xlsx?cb=' + cacheBuster;
