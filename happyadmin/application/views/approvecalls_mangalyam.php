    ...
    <link rel="stylesheet" href="<?php echo base_url() ?>assets/css/custom.css">
        ...
        <div>
         ...
         <?php $approvecalls_count = $this->db->select('tr.*,tasp.action_check,tasp.assign_id,tasp.active_status,tasp.goto_status')
                                      ->from('tbl_registration as tr')
                                    ->join('tbl_assign_approve_calls as tasp','tasp.action_check = tr.id')
                                    ->where('active_status!=','Inactive')
                                    ->where('goto_status','2')
                                    ->where('assign_id',$sess_id)->where('status',1)->get()->num_rows();?>
   ...
   <!--<?php echo $this->session->userdata('username');?>-->
      ...
      <button type="submit" class="btn btn-primary"  id="btn_update" style="color:#fff;">Save changes
        ...
        <form id="contact" method="post" action="<?php echo base_url(); ?>admin/payment_request_approve_calls" onsubmit="setTimeout(function(){window.location.reload();},700);">
