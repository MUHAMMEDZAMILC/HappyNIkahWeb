...
<link rel="stylesheet" href="<?php echo base_url() ?>assets/css/custom.css">
        ...
        <div>
         ...
         <?php $activecalls_count = $this->db->select('tr.*,tasp.action_check,tasp.assign_id,tasp.active_status')->from('tbl_registration as tr')
        ...
        <h6 style="float: right;">Total records : <?php echo $activecalls_count;?></h6>
        </div>
        <br><br>
        ...
        <?php $sess_id= $this->session->userdata('user_id_admin');?>
             ...
             <?php $activecalls = $this->db->select('tbl_registration.*,tbl_assign_active_calls.action_check,tbl_assign_active_calls.active_status')->from('tbl_registration')
                                   ->join('tbl_assign_active_calls','tbl_assign_active_calls.action_check=tbl_registration.id')
                                   ->where('assign_status!=','Inactive')
                                    ->where('active_status!=','Inactive')
                                   ->get()->result_array();?>
                                   

        <?php foreach($activecalls as $approve)
        {?>
