...
<link rel="stylesheet" href="<?php echo base_url() ?>assets/css/custom.css">
        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            $this->db->where('tbl_payment_viewed.from','happymangalyam');
        ...
        $this->db->select('tbl_registration.*,tbl_payment_viewed.contact_viewed_id,tbl_payment_viewed.sender_id,tbl_payment_viewed.date,tbl_payment_viewed.planid,tbl_payment_viewed.visit_count');
        $this->db->from('tbl_payment_viewed');
        $this->db->join('tbl_registration','tbl_registration.id=tbl_payment_viewed.sender_id');
        ...
        <span id="" style="color:#8f0606;"><b>Contact Number : <span><?php echo $value['phone'];?></span></b></span>
         ...
         <?php $sender_id= $value['sender_id'];?>
        <?php $paidstatus=$this->db->select('user_id,status')->from('tbl_payement')->where('status','1')->where('user_id',$sender_id)->get()->row_array();?>
        
        <?php $userpaid = $paidstatus['user_id'];?>
        <?php if($userpaid==$sender_id)
        {?>
       
        
         <span class="countcircle" style="background: #ff3bff;">
            <?php echo "Paid";?>  
        </span>
        <?php }?>
       ...
       <?php $dateString =$value['date'];?>
        ...
        <form method="post" action="<?= base_url() ?>admin/paymentvisit_mangalyam_delete">
